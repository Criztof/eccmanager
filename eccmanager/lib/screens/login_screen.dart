...
class _LoginScreenState extends State<LoginScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      String rol = await _authService.obtenerRolUsuario(userCredential.user!.uid);
  ...
  Future<void> _resetPassword() async {
    ...
    // Mostramos un indicador de carga si prefieres, o simplemente ejecutamos
    try {
      await _authService.recuperarPassword(email);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Enlace enviado. Revisa tu correo (spam incluido).'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(e.message ?? 'Error al procesar la solicitud'),
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              Hero(
                ...
                child: Container(
                  ...
                  child: Padding(
                    ...
                    child: Image.asset('assets/logo_ecc.jpeg', fit: BoxFit.contain),
                  ),
                ),
              ),
              ...
              const FadeSlideTransition(
                delay: 100,
                child: Text('Inicia sesión en eccmanager', style: TextStyle(color: Colors.grey)),
              ),
              const SizedBox(height: 40),
              FadeSlideTransition(
                delay: 200,
                child: _CustomTextField(controller: _emailController, icon: Icons.email_outlined, hint: 'Correo electrónico'),
              ),
              const SizedBox(height: 16),
              FadeSlideTransition(
                delay: 300,
                child: _CustomTextField(controller: _passwordController, icon: Icons.lock_outline, hint: 'Contraseña', isPassword: true),
              ),
              Align(
                alignment: Alignment.centerRight,
                child: TextButton(
                  onPressed: _resetPassword,
                  child: const Text('¿Olvidaste tu contraseña?', style: TextStyle(color: Color(0xFF1B5E20), fontWeight: FontWeight.w600)),
                ),
              ),
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    GestureDetector(
                      ...
                      child: const Text('Regístrate', style: TextStyle(color: Color(0xFF1B5E20), fontWeight: FontWeight.bold)),
                    ),
                  ],
                ),
              ),
